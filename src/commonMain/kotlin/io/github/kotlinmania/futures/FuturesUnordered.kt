...
/**
 * An adapter for [Stream] which yields the outcomes of up to [n] futures concurrently.
 */
@HiddenFromObjC
public fun <T> Stream<Future<T>>.bufferUnordered(n: Int): Stream<T> {
    require(n > 0) { "limit must be greater than 0" }
    val upstream = this
    val queue = FuturesUnordered<T>()
        ...
        override fun pollNext(context: TaskContext): Poll<Yield<T>> {
            while (!upstreamDone && queue.len() < n) {
                when (val p = upstream.pollNext(context)) {
                    is Poll.Ready -> {
                        when (val y = p.value) {
                            is Yield.Value -> queue.push(y.value)
                            Yield.End -> {
                                upstreamDone = true
                                break
                            }
