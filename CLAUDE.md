# Claude Code Project Instructions — futures-kotlin

## Project Overview

This is **futures-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`futures`](https://crates.io/crates/futures).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/futures/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/futures>
