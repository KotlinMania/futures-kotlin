...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`futures`](https://crates.io/crates/futures).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `futures` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Composable asynchronous programming primitives
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:futures-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `futures` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
